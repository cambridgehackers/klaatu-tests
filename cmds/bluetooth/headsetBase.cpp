...
static inline native_data_t * get_native_data() {
    return (native_data_t *)NULL;
}
...
static inline int write_error_check(int fd, const char* line, int len) {
    ...
    if (ret < 0) {
        ALOGE("%s: write() failed: %s (%d)", __FUNCTION__, strerror(errno), errno);
        return -1;
    }
    if (ret != len) {
...
again:
    ...
    *err = errno = 0;
    ...
    while ((int)(bufit - buf) < (len - 1)) {
        ...
        if (rc < 0) {
            if (errno == EBUSY) {
                ALOGI("read() error %s (%d): repeating read()...", strerror(errno), errno);
                goto again;
            }
            *err = errno;
...
static void initializeNativeDataNative(int socketFd) {
    ...
    //nat->address = (String8)env->NewGlobalRef(env->GetObjectField(object, field_mAddress));
...
static bool connectNative()
{
    ...
    while (nat->rfcomm_connected == 0) {
        if (connect(nat->rfcomm_sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
...
static int connectAsyncNative() {
    ...
    if (nat->rfcomm_sock_flags >= 0) {
        ...
        if (fcntl(nat->rfcomm_sock, F_SETFL, nat->rfcomm_sock_flags | O_NONBLOCK) >= 0) {
            int rc;
            nat->rfcomm_connected = 0;
            errno = 0;
            rc = connect(nat->rfcomm_sock, (struct sockaddr *)&addr, sizeof(addr)); 
            if (rc >= 0) {
                nat->rfcomm_connected = 1;
                ...
                if (errno == EINPROGRESS || errno == EAGAIN) {
                    ALOGI("async connect is in progress (%s)", strerror(errno));
                    nat->rfcomm_connected = -1;
                    return 0;
                }
                else {
...
static int waitForAsyncConnectNative(int timeout_ms) {
    struct sockaddr_rc addr;
    native_data_t *nat = get_native_data(); 
    ...
    if (nat->rfcomm_sock >= 0) {
        ...
        n = select(nat->rfcomm_sock + 1, &rset, &wset, NULL, (timeout_ms < 0 ? NULL : &to)); 
        if (timeout_ms > 0) {
            int remaining = to.tv_sec*1000 + to.tv_usec/1000;
            ALOGV("Remaining time %ldms", (long)remaining);
            //env->SetIntField(obj, field_mTimeoutRemainingMs, remaining);
        }

        if (n <= 0) {
            if (n < 0)  {
                ALOGE("select() on RFCOMM socket: %s (%d)", strerror(errno), errno);
                return -errno;
            }
            return 0;
        }
        ...
        if (FD_ISSET(nat->rfcomm_sock, &rset) || FD_ISSET(nat->rfcomm_sock, &wset)) {
    ...
    else ALOGE("RFCOMM socket file descriptor %d is bad!", nat->rfcomm_sock);
    return -1;
}

static void disconnectNative() {
    native_data_t *nat = get_native_data();
    if (nat->rfcomm_sock >= 0) {
        close(nat->rfcomm_sock);
        nat->rfcomm_sock = -1;
...
static bool sendURCNative(String8 urc) {
    native_data_t *nat = get_native_data();
    if (nat->rfcomm_connected) {
        const char *c_urc = urc.string();
        bool ret = send_line(nat->rfcomm_sock, c_urc) == 0 ? TRUE : FALSE;
        if (ret == TRUE) pretty_log_urc(c_urc);
...
static String8 readNative(int timeout_ms) {
        native_data_t *nat = get_native_data();
    String8 retval;
        if (nat->rfcomm_connected) {
            char buf[256];
            const char *ret = get_line(nat->rfcomm_sock, buf, sizeof(buf), timeout_ms, &nat->last_read_err);
            retval = String8(ret);
        }
        return retval;
}

static int getLastReadStatusNative() {
        native_data_t *nat = get_native_data();
        if (nat->rfcomm_connected)
            return (int)nat->last_read_err;
        return 0;
}
